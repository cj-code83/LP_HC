...
interface AuditProps {
  ...
  onAnswerChange: (id: string, answer: "yes" | "no" | "na") => void;
...
export default function Audit({
  ...
  const canSubmit = answeredCount === totalQuestions && leaderName.trim();

  return (
    <div className="space-y-4">
      {/* Leader info */}
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        <h2 className="text-sm font-semibold text-card-foreground mb-3">Audit Details</h2>
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-1">
            Leader Name <span className="text-primary">*</span>
          </label>
          <input
            type="text"
            value={leaderName}
            ...
            {isExpanded && (
              <div className="divide-y divide-border border-t border-border">
                {catQuestions.map((q, idx) => {
                  ...
                  const rowBg =
                    answer === "yes" ? "bg-emerald-50/50 dark:bg-emerald-900/10" :
                    answer === "no"  ? "bg-red-50/50 dark:bg-red-900/10" :
                    answer === "na"  ? "bg-muted/30" : "";

                  return (
                    <div key={q.id} className={`p-4 transition-colors ${rowBg}`}>
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          {q.weight === 2 && (
                            <span className="inline-flex items-center gap-0.5 rounded-full bg-orange-100 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 px-1.5 py-0.5 text-[10px] font-bold text-orange-700 dark:text-orange-400 uppercase tracking-wide mb-1">
                              ⚡ Critical
                            </span>
                          )}
                      ...
                      <div className="flex gap-2 pl-9">
                        <button
                          ...
                          className={`answer-yes flex-1 flex items-center justify-center gap-1.5 rounded-xl py-3 text-sm font-semibold transition-all active:scale-[0.97] ${
                            answer === "yes"
                        ...
                        <button
                          ...
                          className={`answer-no flex-1 flex items-center justify-center gap-1.5 rounded-xl py-3 text-sm font-semibold transition-all active:scale-[0.97] ${
                            answer === "no"
                        ...
                        <button
                          onClick={() => onAnswerChange(q.id, "na")}
                          className={`flex items-center justify-center gap-1.5 rounded-xl py-3 px-3 text-sm font-semibold transition-all active:scale-[0.97] ${
                            answer === "na"
                              ? "bg-muted-foreground text-white shadow-md"
                              : "border-2 border-border text-muted-foreground hover:bg-muted/50"
                          }`}
                        >
                          <MinusCircle size={15} />
