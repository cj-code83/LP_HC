...
export default function Results({ result, isReadOnly = false, onNewAudit, onDashboard }: ResultsProps) {
  ...
  return (
    <div className="space-y-5">
      {isReadOnly && (
        <div className="flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-3 shadow-sm text-sm text-muted-foreground">
          <span className="text-base">📋</span>
          <span>Viewing a saved audit record</span>
        </div>
      )}
      ...
      <div className={`rounded-2xl bg-gradient-to-br ${cfg.gradientFrom} ${cfg.gradientTo} p-1 shadow-lg`}>
        <div className="rounded-xl bg-card/95 dark:bg-card p-5 text-center">
          ...
          <div className="mt-4 flex items-center justify-center gap-1.5 text-sm">
            ...
            {result.passed !== result.passedCount && (
              <span className="text-muted-foreground">
                ({result.passed}/{result.total} weighted pts)
              </span>
            )}
          </div>
          ...
          <button
            onClick={handlePrint}
            className="no-print mt-4 inline-flex items-center gap-2 rounded-lg border border-border bg-background px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-muted/60 transition-colors active:scale-[0.97]"
          >
            <Printer size={14} />
            Print / Save as PDF
          </button>
      ...
      {failedQuestions.length > 0 && (
        <div className="rounded-xl border border-red-200 dark:border-red-800 bg-card shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 bg-red-50 dark:bg-red-900/20 border-b border-red-200 dark:border-red-800">
            <AlertTriangle size={16} className="text-primary dark:text-red-400" />
            <h3 className="text-sm font-semibold text-primary dark:text-red-300">
          ...
          <ul className="divide-y divide-border">
            {failedQuestions.map((q) => (
              <li key={q.id} className="flex items-start gap-3 px-4 py-3">
                <XCircle size={16} className="shrink-0 mt-0.5 text-primary" />
                <div>
      ...
      <div className="flex gap-3 pb-6 no-print">
        <button
          onClick={onDashboard}
          className="flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-3.5 text-sm font-semibold text-muted-foreground hover:bg-muted/50 transition-colors active:scale-[0.97]"
        >
        ...
        <button
          onClick={onNewAudit}
          className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-primary py-3.5 text-sm font-semibold text-primary-foreground shadow-md hover:opacity-90 transition-all active:scale-[0.98]"
        >
          <RotateCcw size={15} />
          {isReadOnly ? "Start New Audit" : "New Audit"}
        </button>
      </div>
    </div>
  );
